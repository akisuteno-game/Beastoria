...
export function renderRoster(container, roster, allSpecies, onSelect) {
  ...
  // 図鑑は種族ごとに1枠だけ登録する(同じ種族を何体持っていても1枚)
  const seenSpeciesIds = new Set();
  const representatives = [];
  roster.list.forEach((instance) => {
    if (!seenSpeciesIds.has(instance.speciesId)) {
      seenSpeciesIds.add(instance.speciesId);
      representatives.push(instance);
    }
  });

  representatives.forEach((instance) => {
    container.appendChild(buildOwnedCard(instance, onSelect));
  });

  allSpecies
    .filter((species) => !seenSpeciesIds.has(species.id))
